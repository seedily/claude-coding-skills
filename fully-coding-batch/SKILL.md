...
description: 超长 / 多功能需求的批次串行开发工具。适用于"一次要实现多个模块 / 大型需求分批开发 / 多功能并行交付"等场景。自动拆分子任务并分析依赖，逐个调用 fully-coding 流水线（Step 3-9），共享月度大版本分支与 project-config 配置，支持断点续跑
---
...
2. `rules/batch-runtime-load-map.md`：按场景加载最小上下文（运行时按场景懒加载以此为准）。
...
> 以上"运行必读顺序"为入口概览；实际运行时按场景懒加载以 `rules/batch-runtime-load-map.md` 为准，二者并不冲突。
