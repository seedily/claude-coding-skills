...
| 进度监控 | batch Orchestrator | `agents/batch-orchestrator.md` Step 3; `rules/batch-workflow-rules.md` §4; `BATCH-{batch-ts}-progress.md` | 子任务 `{tsN}-codingLog.md`; `{tsN}-blockLog.md` |
...
- **子任务公共文档防覆盖约束**：`../../fully-coding/rules/runtime-load-map.md` 末尾「batch 子任务额外必读」作用于 fully-coding 子任务进程（由子任务 `codingLog` frontmatter `batch-subtask: true` 触发），batch Orchestrator 只负责传入 `--batch-subtask`，不替子任务加载或改写公共知识库文档。
