...
5. ✅ **必读并遵循** `rules/backend-code-standard-rules.md`（命名、Lombok、日志、并发、**接口契约**）与（前端工程）`rules/frontend-code-standard-rules.md`（**API 命名/URL**）；按需加载 `rules/persistence-sql-cache-rules.md`（缓存、Mapper SQL、分页、索引）、`rules/security-rules.md`（SQL 注入、XSS、越权、敏感信息）、`rules/error-handling-rules.md` 等编码规范
