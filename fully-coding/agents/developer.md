...
> 按 `rules/runtime-load-map.md` 懒加载 DDD、编码、持久层、安全、异常等完整规则；实现涉及对应风险或细节时再读取。

### 必做动作 Checklist
1. ✅ 通读 `{{config.output_dir}}/{ts}-codingLog.md` 的步骤 3、4
2. ✅ 通读对应的 `{{config.knowledge_dir}}/{{config.feature_impl_glob}}`
3. ✅ 使用 Glob / Grep 扫描项目已有代码，找到同类代码作为参考
4. ✅ 按需加载并遵循 `rules/backend-ddd-layer-rules.md`：
...
5. ✅ 按需加载并遵循 `rules/backend-code-standard-rules.md`（命名、Lombok、日志、并发等）、`rules/persistence-sql-cache-rules.md`（缓存、Mapper SQL、分页、索引）、`rules/security-rules.md`（SQL 注入、XSS、越权、敏感信息）、`rules/error-handling-rules.md` 等编码规范
6. ✅ 涉及错误码时遵循 `{{config.knowledge_dir}}/{{config.error_code_doc}}` 和 `rules/error-handling-rules.md`：
   - 优先使用已有 `[L][CC]` 枚举
   - 使用当前系统的 `SS` 与 `D`
   - 若新增，在 `codingLog.md` 步骤 5 记录「新增错误码：取值/含义/使用场景」
7. ✅ 使用 `{{config.error_code_utility}}` 生成错误码（如项目存在该文件，确保 SYSTEM_CODE 与 SS 一致）
8. ✅ 执行本地构建验证（Maven/Gradle compile/test）
9. ✅ 将代码变更清单与编译结果写入 `{{config.output_dir}}/{ts}-codingLog.md`
...
- **阻塞登记**：若步骤 5 或步骤 7 发生阻塞且流程无法继续、必须用户确认时（如编译错误无法修复、测试失败超出自动修复能力），才登记：
