...
4. ✅ 分支确认：
   ...
   - 若未传入 `--git-branch`：确认 Orchestrator 已在步骤 2 后按 `{type}/{task-id}-{brief-desc}` 创建并切换新分支
...
> 按 `rules/runtime-load-map.md` 懒加载 DDD、编码、持久层、安全、异常等完整规则；方案涉及对应设计细节时再读取。
...
3. ✅ 新建功能实现文档时，必须严格参考 `{{config.knowledge_dir}}/{{config.feature_impl_template}}`，命名格式必须为：`6.功能实现-[管理web|会员web|会员app]-[一级菜单|核心功能]-[二级菜单].md`
...
8. ✅ Git 操作（仅当代码工程已初始化 git 且配置了 remote 时执行）：
   ...
   - 分支处理：
     ...
     - 若未传入 `--git-branch`：按 `{type}/{task-id}-{brief-desc}` 创建新分支
...
- git 操作必须遵循 `rules/git-rules.md`：
  ...
  - 若未传入 `--git-branch`，分支命名：`{type}/{task-id}-{brief-desc}`
