...
- 根据参数选择执行模式：标准模式 Step 1-9、`--plan-only` Step 1-4、`--quick-dev` 最小需求记录 → Step 3 → Step 5 → Step 6 → Step 7。
...
3. 若未传入 `--new-task` 且未传入 `--start-step`、`--plan-only`、`--quick-dev`，扫描 `{{config.output_dir}}/*-codingLog.md`。
4. 按 `rules/heartbeat-resume-rules.md` 判断任务是否可接管。
5. 恢复前先检查 `{ts}-blockLog.md`；存在「用户确认状态：待确认」时不得恢复。
6. 新任务默认从 Step 1 开始；batch 子任务可按参数从 Step 3 开始；`--plan-only` 从 Step 1 开始并停在 Step 4；`--quick-dev` 写最小需求记录后从 Step 3 进入。
